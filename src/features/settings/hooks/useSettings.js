...
export function useSettings() {
  ...
  useEffect(() => {
    ...
    loadSettings().then(async (nextSettings) => {
      ...
      if (
        nextSettings?.background?.type === "image" &&
        nextSettings.background.source === "live"
      ) {
        const wallpaper =
          (await getCurrentWallpaper()) || (await refreshDailyWallpaper(false));
        if (wallpaper?.url) {
  ...
  useEffect(() => {
    if (
      ...
      settings.background.source !== "live"
    )
      return;
    ...
    const handleChanged = (changes, areaName) => {
      ...
      setSettings((current) => {
        if (
          ...
          current.background.source !== "live"
        )
          return current;
