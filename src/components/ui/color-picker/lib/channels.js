 ...
 */
export function colorChannels(color, format) {
  switch (format) {
    case "hex":
      return [];
    case "rgb": {
 ...
 */
export function setColorChannel(color, format, key, value) {
  if (key === "alpha") {
  ...
  switch (format) {
    ...
    case "hsl": {
      ...
      return fromCulori({ mode: "hsl", h, s, l }, color.alpha);
    }
    case "hsb": {
      const hsv = toHsv({
        mode: "oklch",
      ...
      return fromCulori({ mode: "hsv", h, s, v }, color.alpha);
    }
    case "oklch": {
      switch (key) {
        case "l":
    ...
    case "oklab": {
      ...
      return fromCulori({ mode: "oklab", l, a, b }, color.alpha);
    }
    case "p3": {
      const p3 = toP3({
        mode: "oklch",
...
function intChannel(key, label, value, min, max, suffix) {
  return {
    key,
    label,
    value,
...
function floatChannel(key, label, value, min, max, step, bigStep, precision, suffix) {
